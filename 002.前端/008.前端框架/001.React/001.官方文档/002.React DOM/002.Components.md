...
- `angerouslySetInnerHTML`：形式为 `{ __html: '<p>some html</p>' }` 的对象，内部包含原始 HTML 字符串
	...
	- 使用此方法时应格外小心！如果内部的 HTML 不受信任（如：基于用户数据），则将面临引入 [[014.XSS 攻击|XSS]] 漏洞的风险
- `ref`：来自 [[002.前端/008.前端框架/001.React/001.官方文档/001.React/001.Hooks#useRef|useRef]] 或 [`createRef`（过时）](https://react.dev/reference/react/createRef) 的 ref 对象，或 [`ref` 回调函数](https://react.dev/reference/react-dom/components/common#ref-callback)，或 [ref 字符串（过时）](https://reactjs.org/docs/refs-and-the-dom.html#legacy-api-string-refs)。`ref` 将填充该节点的 DOM 元素
