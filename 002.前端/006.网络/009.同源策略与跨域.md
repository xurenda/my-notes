...
[[019.CORS 预检请求的 TTL 缓存机制|CORS 预检请求的 TTL 缓存机制]]
...
`script` 标签的 `crossOrigin` 属性具有以下的值：

- `anonymous`：进行跨源请求，但不发送用户凭据（如 Cookie、HTTP 认证信息等）
- `use-credentials`：进行跨源请求，并发送用户凭据

虽然 `script` 标签不受同源限制，但也有一些功能限制，例如无法拿到加载错误信息，不能对 DOM 进行操作，此时 `crossOrigin` 字段可以解决这些问题：
