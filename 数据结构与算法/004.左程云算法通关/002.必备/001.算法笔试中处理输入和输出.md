...
[[学习规划#Step.2 卡码网|核心代码模式和 ACM 模式]]
...
`bufio.NewScanner` 缓冲区默认为 `64*1024` 字节，当一行数据多于缓冲区大小时会 `panic`，使用 `bufio.NewReaderSize` 指定缓冲区大小，如：[[020.删除重复元素#答案|这道题]]
...
func main() {
    ...
    for input.Scan() {
        ...
        output.WriteString(strconv.Itoa(a + b)) // 先写出到缓冲区
        output.WriteByte('\n')
    }
