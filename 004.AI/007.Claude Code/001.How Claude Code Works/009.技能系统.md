...
2. 项目技能：工作区级（projectSettings），路径：`.claude/skills/`
3. 用户技能：用户级（userSettings），路径：`~/.claude/skills/`
4. 插件技能（plugin）：通过插件市场安装的，从已启用插件加载
5. 内置技能（bundled）：
	- 代码里写死的，如 `/simplify, /debug, /verify`
	- `registerBundledSkill()` 启动注册
6. MCP 技能：MCP 服务器提供的 prompts，通过 `mcpSkillBuilders` 暴露
