...
![[Pasted image 20250901192137.png]]

# 大模型的输出机制

在 KV cache 的支持下，大模型生成 token 分为两个阶段：
...
一次完整的前向传播（input token 数量）与多次利用 KV cache 的前向传播（output token 数量），哪个开销更大呢？

![[Pasted image 20250901192456.png]]
