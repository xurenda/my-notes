...
modified: 2024-08-13 23:55
---

# createContext

在==组件外部==调用 `createContext` 以创建上下文，与 [[001.Hooks#useContext|useContext]] 一起使用
...
将组件==包装==在 `forwardRef` 中以转发 Ref，使组件通过 Ref 向父组件公开 DOM 节点或自定义命令式句柄（通过 [[001.Hooks#useImperativeHandle|useImperativeHandle]]）
...
- `SomeComponent`：一个可以在树中渲染的 React 组件。当惰性组件的代码仍在加载时，尝试渲染它将会挂起（使用 [[002.Components#`<Suspense>`|<Suspense>]] 在加载时显示加载指示器）
...
- prop 是对象：可以在组件外部声明它以使其永远不会更改，或使用 [[001.Hooks#useMemo|useMemo]] 防止父组件每次渲染都创建新对象
- prop 是函数：可以在组件外部声明它以使其永远不会更改，或使用 [[001.Hooks#useCallback|useCallback]] 防止父组件每次渲染都创建新函数
...
将状态更新标记为非紧急，类似于 [[001.Hooks#useTransition|useTransition]]

```js
startTransition(scope)
```

- `scope`：通过调用一个或多个 `set` 函数来更新某些状态的函数，React 立即调用不带参数的 `scope` ，并将 `scope` 函数调用期间同步计划的所有状态更新标记为转换（非阻塞 UI、不会 [[002.Components#`<Suspense>`|<Suspense>]] fallback）
...
- `startTransition` 不提供跟踪 Transition 是否处于挂起状态的方法。要在转换正在进行时显示待处理指示器，使用 [[001.Hooks#useTransition|useTransition]]
- 仅当有权访问该状态的 `set` 函数时，才可以将更新包装到 Transition 中。如果想启动 Transition 来响应某些 prop 或自定义 Hook 返回值，使用 [[001.Hooks#useDeferredValue|useDeferredValue]]
...
- 当使用 Promise 调用时，`use` 会与 [[002.Components#`<Suspense>`|Suspense]] 和[错误边界](https://react.dev/reference/react/Component#catching-rendering-errors-with-an-error-boundary)集成
	- 挂起时：显示 Suspense 的后备
	- 拒绝时：显示最近的错误边界的回退
- 当资源是 context 时，`use` 的工作方式与 [[001.Hooks#useContext|useContext]] 类似，但其可以在循环和条件语句内调用
