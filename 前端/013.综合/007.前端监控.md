...
modified: 2025-07-11 10:12
---

出处：[掘金](https://juejin.cn/post/7482957797770756131)
...
> [!NOTE] 个人经验
> 
> 阅读本文可以了解下原理，正常情况下，小公司一般会直接买现成的服务，而不是自己搭，做这么一套下来的成本很大（尤其是数据测）

在大型项目中，前端监控是不可或缺的一部分。它的优势可以体现在以下场景：
...
![[Pasted image 20250710194931.png]]

其中：

- SDK：负责处理客户端（浏览器）程序在运行期间的监控日志收集和上报
- 日志服务器，负责接收 SDK 上报的 Log 日志，进行清洗过滤存入数据库
- 可视化平台：以可视化形式，直观展示上报过来的数据

下面，我们重点从 0 到 1 一步步来构建一个 Web 监控 SDK

监控 SDK 代码仓库：[https://github.com/L-excellence/monitor-sdk](https://github.com/L-excellence/monitor-sdk)

# 设计 SDK

SDK 承担了==前端监控和数据上报==的工作，通常我们会以 `<script>` JS 脚本文件的形式接入到业务项目中

首先建立一个 SDK 项目仓库，利用打包工具（如 `Rollup`）构建出 JS 脚本文件

## 搭建 Rollup 构建环境

`Rollup` 非常适合构建一些工具库、组件库。初始化一个 `monitor-sdk` 目录，并完成 Rollup 打包配置：
...
在 `package.json` 中加入 `build` 命令，运行 `npm run build` 完成 `dist/myMonitor.js` 构建：
...
另外可以配置 ESLint、Prettier 等前端规范工具，由于非本文中心主题，跳过详细描述

## 异常监控

在前端，程序发生异常的种类有很多，比如：JS 代码执行错误、Promise 未被处理的错误、React/Vue 组件 render 错误、静态资源加载错误、请求 API 错误等

以上这些异常场景，都需要进行监控并上报到服务器
...
![[Pasted image 20250710195747.png]]
...
> [!tip] 注意
> 
> 注意是在错误事件传播的==捕获阶段==进行接听，避免错误在冒泡过程中可能被其他代码干扰或覆盖

其中 `errorType` 标识是一个 JS 代码错误（`"jsError"`），`message`、`filename`、`position` 信息都可以从 `event` 错误事件对象上获取
...
![[Pasted image 20250710200419.png]]

调用 `formatStack()` 稍做加工一下，得到一个更直观的调用栈的信息，加工后展示如下：

![[Pasted image 20250710200436.png]]
...
![[Pasted image 20250710200810.png]]
...
![[Pasted image 20250710200943.png]]
...
1. `ajax` 传统与服务端通信请求
2. 图片 GET 请求方式上报，优点：速度快，没有跨域问题
3. `navigator.sendBeacon()` 方法通过 HTTP 将少量数据异步传输到 Web 服务器

`ajax` 不多介绍

[`navigator.sendBeacon()`](https://developer.mozilla.org/zh-CN/docs/Web/API/Navigator/sendBeacon) 也是一种不错的方式，它的最大优势在于数据在页面卸载过程中仍然能够被发送，而不会被中断

重点介绍一下 `img GIF` 的上报方式：在服务器上放置一个非常小的图片，将日志数据作为图片 `url queryString` 参数，访问这个图片来达成数据上报至服务器
...
这里 `baseData` 表示日志上报可以携带一些设备的基础信息，比如浏览器型号、版本等。此外，还可以加入一些业务信息（如：用户身份信息）进行上报
...
2. `reject()`：`event.reason` 是一个字符串，值为传递给 `reason()` 函数的失败原因参数
...
资源一般是指 JS、CSS、图片等，当访问这些资源出错或 404 找不到时，我们可以监控进行上报

如下，我们随意访问一个不存在的资源 `/404.js`，模拟资源加载 404
...
以 `xhr` 为例，要监控 API 请求的状态，推荐的做法是采用==重写 `XMLHttpRequest` 构造函数原型上的方法==来实现
...
3. 重写 `XMLHttpRequest.prototype.send` 方法，当判定请求出错时（如：跨域、404、500），收集请求信息进行数据上报

另外，我们还需要配置一个白名单 `whiteList`，可跳过无需监控的请求
...
白屏通常是由代码执行错误引起，导致框架（如 React）渲染流程中断。所以白屏可以结合【异常监控】一起上报，可以关联到导致白屏的错误原因

思考一下：如何判定当前页面是白屏呢？

可以采用【页面采样识别检测】：通过在页面上确定多个采样点，使用 `elementFromPoint` 方法获取采样点的元素，判断采样点元素是否为有效元素（如非 `body、#root` 等根节点）

关于采样点的定义，以页面为中心的水平和垂直线上，来定义多个采样点：
...
页面加载时间的指标信息有很多，一般会重点分析 DOM 树构建完成的时间（`DOMContentLoaded`） 和页面完整的加载时间（`load`）

浏览器 [`PerformanceNavigationTiming`](https://developer.mozilla.org/zh-CN/docs/Web/API/PerformanceNavigationTiming) 对象提供了关于页面加载性能的详细信息（对应旧版本的 `performance.timing` 对象）

在统计加载时间前，先了解两个 JS 事件的执行时机：
...
![[Pasted image 20250710203754.png]]
...
2. FCP, First Content Paint（首次内容绘制）：是浏览器将第一个有内容的 DOM 渲染到屏幕的时间，内容可以是文本、图像、SVG 等，这其实就是白屏时间
3. FMP, First Meaningful Paint（首次有意义内容绘制）：页面有意义的内容（由我们指定）渲染的时间；
4. LCP, Largest Contentful Paint（最大内容渲染）：代表的是视窗最大可见图片或者文本块的渲染时间（以百度首页为例，LCP 对应的元素是百度 Logo）
...
这里先认识一个 HTML 属性：`elementtiming` 属性用于标记页面中特定元素（如图片、视频、文本块等），以便通过 `PerformanceObserver` 的 `element` 类型监控这些元素的加载和渲染时间
...
如下示例，我们在 1.5s 后向页面添加有意义（属性标识）的元素：

```html
<script>
// 1.5s 后向页面添加有意义（属性标识）的元素
setTimeout(() => {
...
使用 PerformanceObserver API 观察 `type: element` 来计算 FMP：
...
新建 `module/longTask.ts` 文件，监听长任务（longtask）作为卡顿现象进行数据上报：
...
在前端项目中，为了节省构建资源体积，不暴露业务逻辑，都会选择将代码进行混淆和压缩。但在优化性能和提升用户体验的同时，也为异常的处理带来了麻烦

Source Map 是一个源代码信息文件，里面存储着代码压缩混淆前后的对应关系。输入混淆后的行列号，就能够获得对应的原始代码的行列号，结合源代码文件便可定位到真实的报错位置

但在生产环境下，不建议将 Source Map 放到网站上，对于具有一定规模和保密性的项目，这样等于将页面逻辑直接暴露给了网站使用者
...
比如可以使用 `name + message + stack` 作为聚合依据，生成一个 `hash` 值来作为这个错误的唯一性 ID

如果后续捕获到的错误生成的 `hash` 值与之前某个错误的 `hash` 值相同，则认为是重复错误，不再进行数据入库或者是统计错误发生次数 + 1

## 数据加工和清洗

数据加工和清洗发生在服务端。在对数据进行入库前，进行数据加工和清洗的意义在于：

- 数据加工：可以对数据提取出关键指标和有价值的信息，以及扩展一些额外信息（如：在服务端才能获取的 IP 地址）
...
错误报警可以从两个维度进行：对新增异常的报警、对错误指标的数量（达到某个阈值）的报警

常见的报警的方式：邮件报警、接入办公即时通信应用（飞书、钉钉）等
