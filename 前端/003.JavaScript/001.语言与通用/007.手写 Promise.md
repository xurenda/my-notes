...
- 参数接收一个可迭代对象（如：数组），包含多个 `Promise` 或其他类型（其他类型会通过 `Promise.resolve()` 包裹转换为 `Promise`）
- 数组中所有 `Promise` 执行完成之后才会返回结果（全部 `resolve` 则 `resolve`；有一个 `reject` 就 `reject`）
- 即使有 `Promise` 是 `reject`，其他 `Promise` 也==不会停止执行==。因为 `Promise` 在创建之初（实例化的时候）就已经执行了
