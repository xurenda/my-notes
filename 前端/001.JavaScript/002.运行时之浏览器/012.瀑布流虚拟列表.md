...
Back：[[008.高性能渲染十万条数据|高性能渲染十万条数据]]

前置：[[011.瀑布流|瀑布流]]、[[009.定高虚拟列表|普通虚拟列表]]

# 思路

基本同 [[009.定高虚拟列表|普通虚拟列表]]，只是将 `startIdx` 和 `endIdx` 变成 `startScroll` 和 `endScroll`，记录容器可视区域，依据是否在可视区域内创建渲染列表
